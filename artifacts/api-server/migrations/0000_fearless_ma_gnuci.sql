...
CREATE TYPE "public"."agent_task_status" AS ENUM('pending', 'in_progress', 'completed', 'blocked', 'failed', 'needs_retry');--> statement-breakpoint
...
CREATE TYPE "public"."frequency_category" AS ENUM('healing', 'longevity', 'dna_repair', 'pain_relief', 'relaxation', 'solfeggio', 'rife', 'binaural', 'schumann', 'custom');--> statement-breakpoint
...
CREATE TYPE "public"."waveform_type" AS ENUM('sine', 'square', 'triangle', 'sawtooth');--> statement-breakpoint
...
CREATE TABLE "ai_model_evaluations" (
	...
	"proposed_by_agent" varchar NOT NULL,
...
CREATE TABLE "frequencies" (
	...
	"title" varchar NOT NULL,
	"description" text,
	"frequency_hz" numeric(12, 4) NOT NULL,
	"waveform_type" "waveform_type" DEFAULT 'sine',
...
CREATE TABLE "frequency_categories" (
	...
	"is_active" boolean DEFAULT true,
	"created_at" timestamp DEFAULT now(),
	CONSTRAINT "frequency_categories_name_unique" UNIQUE("name"),
	CONSTRAINT "frequency_categories_slug_unique" UNIQUE("slug")
);
--> statement-breakpoint
...
CREATE TABLE "wp_sync_tracking" (
	...
	"local_checksum" varchar,
	"remote_checksum" varchar,
	"sync_direction" "sync_direction" DEFAULT 'pull',
