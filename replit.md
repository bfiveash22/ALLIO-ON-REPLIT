...
- **PPTX Generator** (`protocol-pptx.ts`): Uses `pptxgenjs` to create 30+ slide presentations matching Trustee's template. Color theme: Navy (#1A2440/#0A0E1A) + Gold (#C9A54E) + Lavender (#E8EDF5/#C8CFE0). Fonts: Playfair Display (headers) + Source Sans 3 (body). Based on `Trustee_Protocol_Template_UPDATE_1773744290526.pptx`. Includes ECS section dividers, suppository slides, sirtuin/mitochondrial slides, liposomal, nebulization, topical, exosome, and dietary protocol slides alongside original sections.
