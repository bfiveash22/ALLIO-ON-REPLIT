# Triage Doctor Intake Procedures
## Complete Guide - Front to Back

**For:** Dr. Baker and Triage Physicians  
**Classification:** Clinical Protocol Document  
**Updated:** March 2026

---

## Overview

This document provides the complete workflow for triage doctor intakes at Forgotten Formula PMA. The triage doctor is the critical first point of clinical contact, responsible for comprehensive assessment and appropriate routing of new members. Following intake, the platform's automated protocol assembly pipeline generates a personalized 90-day healing roadmap delivered as a Google Slides presentation.

---

## Part 1: Pre-Intake Preparation

### 1.1 Before the Appointment

**Review Available Documentation:**
- Membership application and health questionnaire
- Previous medical records (if provided)
- Lab work and imaging (if provided)
- Medication list
- Supplement list
- Chief complaints noted during scheduling

**System Preparation:**
- Open member profile in ALLIO
- Review ATHENA notes from support interactions
- Check for flagged concerns from intake team
- Confirm SignNow membership agreement is signed and verified (enrollment gating)
- Prepare documentation templates

### 1.2 Required Forms Confirmation
Ensure member has completed:
- [ ] Membership Agreement (signed via SignNow — required before enrollment is activated)
- [ ] Health History Questionnaire
- [ ] Current Medications List
- [ ] Supplement List
- [ ] Privacy Acknowledgment
- [ ] Informed Consent for Services

**Note:** The FFPMA platform automatically gates member enrollment. Intake cannot proceed to protocol generation until SignNow contract verification is confirmed in the Doctor Portal.

---

## Part 2: The Intake Consultation

### 2.1 Opening (5 minutes)

**Welcome and Set Expectations:**
"Welcome to Forgotten Formula. I'm Dr. [Name], and I'll be conducting your comprehensive intake today. This will take about 60-90 minutes. We're going to go through everything thoroughly so we can understand your complete health picture and create the best path forward for you."

**Establish Rapport:**
- Acknowledge they've taken a significant step
- Validate their health journey so far
- Create a safe space for honest disclosure

### 2.2 Chief Complaint Deep Dive (15 minutes)

**Primary Concern Assessment:**
1. What is the main issue bringing you to us today?
2. When did this first begin? (Timeline construction)
3. What makes it better or worse?
4. How does this affect your daily life?
5. On a scale of 1-10, how much does this impact your quality of life?
6. What have you already tried? (Treatments, supplements, lifestyle)
7. What do you believe is causing this?

**Secondary Concerns:**
- "What else is bothering you that we should address?"
- "Are there other symptoms you've learned to live with?"
- Capture ALL concerns, even those they've normalized

### 2.3 Complete Systems Review (20 minutes)

**Use the FFPMA 12-System Protocol:**

1. **Neurological**
   - Brain fog, memory, concentration
   - Sleep quality and patterns
   - Mood: anxiety, depression, irritability
   - Neuropathy, numbness, tingling

2. **Cardiovascular**
   - Blood pressure history
   - Palpitations, chest pain
   - Circulation issues
   - Family history of heart disease

3. **Respiratory**
   - Shortness of breath
   - Chronic cough
   - Asthma/allergies
   - Sleep apnea

4. **Gastrointestinal**
   - Digestion quality
   - Bowel movements (frequency, consistency)
   - Bloating, gas, pain
   - Food sensitivities
   - Acid reflux

5. **Genitourinary**
   - Urinary frequency/urgency
   - Infections
   - Sexual function
   - Menstrual history (if applicable)

6. **Musculoskeletal**
   - Joint pain/stiffness
   - Muscle pain
   - Mobility limitations
   - Previous injuries

7. **Dermatological**
   - Skin conditions
   - Rashes, eczema, psoriasis
   - Hair loss
   - Nail changes

8. **Endocrine**
   - Thyroid symptoms
   - Blood sugar regulation
   - Hormone symptoms
   - Energy patterns

9. **Immune**
   - Frequency of illness
   - Autoimmune conditions
   - Allergies
   - Chronic infections

10. **Dental/Oral**
    - Dental work history
    - Root canals, amalgams
    - Gum disease
    - Jaw issues (TMJ)

11. **Sensory**
    - Vision changes
    - Hearing issues
    - Taste/smell changes

12. **Psychological**
    - Stress levels
    - Trauma history
    - Coping mechanisms
    - Support systems

### 2.4 Lifestyle Assessment (10 minutes)

**Nutrition:**
- Typical daily diet (morning to night)
- Water intake
- Alcohol consumption
- Caffeine intake
- Recent diet changes
- Known deficiencies

**Movement/Exercise:**
- Current activity level
- Type of exercise
- Limitations
- Goals

**Sleep:**
- Hours per night
- Sleep quality
- Waking patterns
- Sleep aids used

**Stress & Environment:**
- Work stress level
- Relationship stress
- Environmental exposures
- Living situation
- Chemical exposures (home, work)

### 2.5 Medical History (10 minutes)

**Previous Diagnoses:**
- Chronic conditions
- Acute conditions in past 5 years
- Surgeries (list all)
- Hospitalizations
- Imaging/procedures

**Family History:**
- Parents' health conditions
- Siblings' health conditions
- Grandparents (if known)
- Hereditary patterns

**Medication Review:**
- Current prescriptions (dose, frequency, duration)
- OTC medications
- Supplements (brand, dose)
- Recent changes
- Adherence issues

**Allergy Review:**
- Drug allergies (reaction type)
- Food allergies
- Environmental allergies
- Supplement sensitivities

---

## Part 3: Clinical Assessment

### 3.1 Vitals (5 minutes)
- Blood pressure (both arms if indicated)
- Heart rate
- Temperature
- Respiratory rate
- Pulse oximetry
- Weight
- Height
- BMI calculation

### 3.2 Physical Observations
- General appearance and demeanor
- Skin quality and color
- Visible inflammation markers
- Posture and movement quality
- Eye and nail assessment (if trained)

### 3.3 Vision AI Blood Analysis

**AI-Assisted Blood Panel Review:**
Upload available blood work to the Blood Analysis tool in the Doctor Portal. The platform's Vision AI analyzes blood panels and microscopy images to provide:
- Functional medicine reference range interpretation
- Pattern detection for root-cause correlation
- Cellular health observations
- Protocol recommendations based on findings

**Supported Inputs:**
- Uploaded blood panel images (JPEG, PNG)
- Live microscope capture (dark field microscopy integration)
- Bloodwork lab report PDFs (auto-extracted via AI)

**Preliminary Testing Recommendations:**
Based on intake findings, recommend appropriate testing:

| Finding/Concern | Recommended Testing |
|-----------------|-------------------|
| Fatigue, low energy | CBC, CMP, thyroid panel, iron studies |
| Digestive issues | Comprehensive stool analysis, food sensitivity |
| Hormonal symptoms | Complete hormone panel |
| Cognitive issues | Organic acids, heavy metals |
| Inflammation | hs-CRP, ESR, cytokine panel |
| Cardiovascular concerns | Lipid panel, homocysteine, Lp(a) |

**Live Blood Analysis:**
- Recommend for all new members as baseline
- Provides immediate visual feedback
- Helps guide nutritional recommendations
- Use live capture in the Blood Analysis tool to record video for Drive upload

---

## Part 4: Documentation

### 4.1 ALLIO System Entry

**Required Fields:**
- Chief complaint summary
- Systems review findings
- Current medications
- Allergies (FLAGGED prominently)
- Vital signs
- Assessment notes
- Initial recommendations

**HIPPOCRATES Agent Notification:**
After intake, trigger HIPPOCRATES to:
- Review intake documentation
- Generate preliminary protocol suggestions
- Flag any concerning patterns
- Prepare educational materials for member

### 4.2 Automated Protocol Assembly Pipeline

After completing intake documentation, submit the case to the automated protocol assembly pipeline:

1. **Intake Form Submission** — Submit from the Protocol Assembly page (Doctor Portal > Protocol Assembly). The intake data populates the DR_FORMULA analysis engine.

2. **DR_FORMULA Analysis** — The AI generates a comprehensive 90-day protocol using the FF PMA 2026 methodology, including:
   - Root cause identification (ranked by priority)
   - 5R framework phases (Reduce, Restore, Reactivate, Regenerate, Revitalize)
   - Injectable peptide protocols with reconstitution instructions
   - Supplement stacks with timing and dosing
   - Detox protocols
   - Daily and weekly schedules in checkbox format
   - Molecular pathway citations (PI3K-AKT, NF-κB, etc.)

3. **Google Slides Presentation** — The system automatically generates a formatted presentation and uploads it to Google Drive. A direct link is provided to the doctor for review.

4. **Doctor Review** — The doctor reviews the generated protocol document and makes any clinical refinements.

5. **Trustee/Admin Sign-Off** — Protocols requiring approval are queued for Trustee review via the Protocol Queue tool.

6. **Member Delivery** — Approved protocols are accessible to the member via their member portal and linked presentation.

**AI protocol generation (DR_FORMULA) typically completes within 60 seconds of intake submission. Final delivery to the member follows after doctor review and Trustee/Admin sign-off, which is typically completed within 24 hours of the appointment.**

### 4.3 Member Summary

Provide member with:
- Written summary of intake findings
- Testing recommendations and rationale
- Protocol delivery timeline
- Next steps and follow-up schedule
- Contact information for questions

---

## Part 5: Routing and Follow-Up

### 5.1 Complexity Classification

**Level 1 - Straightforward:**
- Single primary concern
- No major complicating factors
- Clear protocol path
- Route to: Standard practitioner follow-up

**Level 2 - Moderate:**
- Multiple concerns
- Some complexity
- May need multiple modalities
- Route to: Senior practitioner or specialist

**Level 3 - Complex:**
- Multiple chronic conditions
- Polypharmacy
- Autoimmune components
- Previous treatment failures
- Route to: Clinical director or Trustee review

**Level 4 - Critical:**
- Red flags present
- Potential medical emergency
- Beyond PMA scope
- Route to: Immediate conventional care referral

### 5.2 Red Flags Requiring Immediate Action

**STOP and refer to emergency care if:**
- Chest pain with radiation or shortness of breath
- Sudden severe headache ("worst of life")
- Neurological deficits (weakness, slurred speech)
- Severe abdominal pain with rigidity
- Active suicidal ideation with plan
- Signs of acute infection with fever >103°F

### 5.3 Protocol Review and Approval Flow

**Standard Protocol Approval:**
1. Doctor reviews AI-generated protocol in the Protocol Assembly page
2. Protocol enters Trustee queue (viewable at Trustee Dashboard > Protocol Queue)
3. Trustee approves, requests revisions, or escalates
4. Approved protocol is finalized and delivered to member
5. Follow-up scheduled per complexity level

**Expedited Flow (Level 1):**
- Doctor may mark protocol as ready for immediate delivery
- Trustee notified via SENTINEL (Telegram/OpenClaw) for async approval

### 5.4 Follow-Up Scheduling

**Standard Timeline:**
- Testing: Schedule within 1 week
- Results review: 2-3 weeks from testing
- Protocol initiation: After results review (or same day if urgent)
- Progress check: 4-6 weeks into protocol
- LBA follow-up: 4-8 weeks for comparison
- Protocol renewal: At 90-day mark

---

## Part 6: Communication Templates

### 6.1 Post-Intake Summary to Member

```
Dear [Member Name],

Thank you for completing your comprehensive intake today. Here's a summary:

MAIN CONCERNS DISCUSSED:
[List 1-3 primary concerns]

FINDINGS:
[Brief summary of notable findings]

RECOMMENDATIONS:
[Testing recommendations]
[Initial protocol suggestions]

YOUR PROTOCOL:
Our AI system is generating your personalized 90-day protocol. You will receive
a link to your customized presentation within 24 hours of your appointment.

NEXT STEPS:
1. [First step with timeline]
2. [Second step with timeline]
3. [Third step with timeline]

Your next appointment is scheduled for [date/time].

Please contact us with any questions.

In health,
[Doctor Name]
Forgotten Formula PMA
```

### 6.2 Practitioner Handoff Note

```
INTAKE SUMMARY - [Member Name] - [Date]

CC: [Chief complaint in one line]
Duration: [How long]
Impact: [Life impact level 1-10]

KEY FINDINGS:
- [Finding 1]
- [Finding 2]
- [Finding 3]

BLOOD ANALYSIS: [Completed / Pending upload]
VISION AI FINDINGS: [Summary or "See report in chart"]

MEDICATIONS: [List or "See full list in chart"]
ALLERGIES: [List flagged items]

COMPLEXITY: Level [1-4]
RECOMMENDED TRACK: [IV / Peptides / LBA / Detox / ECS / Frequency / etc.]

PROTOCOL STATUS: [Submitted to DR_FORMULA / Awaiting submission]
SLIDES LINK: [Link when available]

URGENT ITEMS: [Any time-sensitive concerns]

- Dr. [Name]
```

---

## Appendix A: Intake Checklist

### Pre-Intake
- [ ] Review member file
- [ ] Confirm SignNow contract verified (enrollment gate)
- [ ] Check form completion
- [ ] Review any previous records
- [ ] Prepare documentation

### During Intake
- [ ] Welcome and set expectations
- [ ] Chief complaint deep dive
- [ ] Complete 12-system review
- [ ] Lifestyle assessment
- [ ] Medical/family history
- [ ] Medication/allergy review
- [ ] Vitals and observations
- [ ] Upload blood work for Vision AI analysis (if available)

### Post-Intake
- [ ] Document in ALLIO
- [ ] Classify complexity level
- [ ] Assign routing
- [ ] Submit intake to Protocol Assembly pipeline
- [ ] Provide member summary
- [ ] Schedule follow-up
- [ ] Notify HIPPOCRATES agent
- [ ] Handoff to assigned practitioner
- [ ] Confirm protocol in Trustee queue

---

## Appendix B: Baker Files Reference

The Baker Files folder in Google Drive contains:
- Sample intake documentation
- Complex case examples
- Protocol templates
- Reference materials
- Training videos

Access via: Allio > Member Content > Baker Files
Or direct link: [To be added by Trustee]

---

## Appendix C: Platform Tools Reference

| Tool | Location | Used For |
|------|----------|---------|
| Blood Analysis | Doctor Portal > Blood Analysis | Upload/capture blood work for Vision AI |
| Protocol Assembly | Doctor Portal > Protocol Assembly | Submit intakes, track protocol status |
| Skin Analysis | Doctor Portal > Diagnostics | Upload skin photos for AI analysis |
| X-Ray Analysis | Doctor Portal > Diagnostics | Upload imaging for AI pattern detection |
| Vitality Assessment | Doctor Portal > Vitality | 7-domain health scoring for members |
| ECS Tool | Member Portal > ECS Tool | Endocannabinoid system profiling |
| Frequency Library | Member Portal > Frequency | Rife/PEMF frequency prescriptions |

---

*Triage Doctor Protocol - Forgotten Formula PMA*  
*Developed with Clinical Oversight*  
*Updated March 2026*
